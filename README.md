...
4. After the first deploy succeeds, open **Shell** on the web service and run:
   ```bash
   python manage.py createsuperuser
   python manage.py seed_site
   ```
5. In Render **Environment**, set `CSRF_TRUSTED_ORIGINS` to your live URL if admin forms fail, e.g.  
   `https://egentidsparesurs.onrender.com`  
   (`RENDER_EXTERNAL_HOSTNAME` is also used automatically in settings.)
6. In Django Admin → **Webbplatsinställningar → SEO**, set **public_site_url** to the same `https://…` URL.
